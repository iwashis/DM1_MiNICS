...
indispensable—summation formulas, which have already been discussed in the lecture.
They are stated once here for quick reference and are \emph{not} re-derived every time
we use them.
...
\medskip
%-----------------------------------------------------------
\item \textbf{Truncated constant sequence (\(a_n=1\) for \(n\le N\), \(a_n=0\) for \(n>N\)).}

Only finitely many terms survive, so the generating function is a polynomial:
\[
f(x)=\sum_{n=0}^{N}x^{n},
\]
which is exactly the finite geometric series \eqref{eq:geom-fin} with \(r=x\):
\[
\boxed{G(x)=\frac{1-x^{N+1}}{1-x}}.
\]
...
\[
\sum_{n=1}^{\infty} a_n x^{n-1} = \frac{1}{x} \left( \sum_{n=1}^{\infty} a_n x^n \right) = \frac{1}{x} \left( \sum_{n=0}^{\infty} a_n x^n - a_0 \right)= \frac{1}{x} \left( f(x) - a_0 \right)
\]
