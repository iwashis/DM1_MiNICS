...
How many ways can we correctly place $n$ pairs of parentheses? This is a classic combinatorial question about \emph{valid parentheses combinations}. For example, with $n=1$ pair, there is only one valid arrangement: \texttt{()}. With $n=2$ pairs, there are two valid arrangements: \texttt{()()} and \texttt{(())}. With $n=3$ pairs, there are five valid arrangements: \texttt{()()()}, \texttt{()(())}, \texttt{(())()}, \texttt{(()())}, \texttt{((()))}. In general, the number of distinct well-formed parentheses sequences grows quickly with $n$.

This problem was our motivation to study a famous sequence of numbers. The numbers counting valid parentheses structures for $n=1,2,3,\dots$ are:

\[1,\; 2,\; 5,\; 14,\; 42,\; 132,\;\dots\]

These are known as the \textbf{Catalan numbers}. In these notes, we will explore Catalan numbers, their recursive definition, how they relate to binary tree structures, and a derivation of their formula using generating functions.
...
Why does the above recurrence hold for valid parentheses? Consider a valid parentheses sequence of $n$ pairs. Focus on the very first \texttt{(} character. It must have a matching \texttt{)}. Say this matching \texttt{)} occurs after forming $k$ pairs inside (between this \texttt{(} and its matching \texttt{)}) -- those $k$ pairs inside must themselves form a valid sequence. The remaining parentheses (after the matching \texttt{)}) will form another valid sequence with $n-1-k$ pairs. See the schematic below for a sequence split by the first pair:
...
Catalan numbers also count the number of distinct \emph{binary tree} structures with a given number of nodes. To appreciate this connection, we first review what a binary tree is:

A \textbf{binary tree} is a hierarchical structure consisting of nodes, where each node may have up to two children: a left child and a right child. A node with no children is called a \emph{leaf}. We often draw binary trees in a planar way with the root at the top, left children branching to the left, and right children to the right. In counting binary trees for Catalan numbers, we consider different shapes of the tree (different arrangements of nodes and child connections) as distinct, but we do *not* label the nodes with any specific values.
...
\begin{enumerate}
  \item A right-skewed chain (root $\to$ right child $\to$ right grandchild).
  \item A tree where the root has only a right child, and that child in turn has a left child.
  \item A balanced tree (root with one left child and one right child).
  \item A tree where the root has only a left child, and that child has a right child.
  \item A left-skewed chain (root $\to$ left child $\to$ left grandchild).
\end{enumerate}
...
\begin{itemize}
  \item 5 of those trees have all 3 of the other nodes in the left subtree (and right subtree empty), and another 5 have all 3 in the right subtree (left empty). These 10 are essentially a root with one side empty and the other side being one of the 5 shapes from the 3-node case.
  \item 2 of the trees have the root with 1 node in the left subtree and 2 in the right subtree.
  \item The remaining 2 have 2 nodes in the left subtree and 1 in the right subtree.
\end{itemize}
...
While the recursive definition of Catalan numbers is useful, we can go further and derive a closed-form formula for $C_n$. A powerful method to solve such recurrences is to use a \textbf{generating function}. Define the generating function $C(x)$ for the Catalan sequence as:
...
There are two solutions, but we must choose the one that gives a valid power series expansion. Since $C(0)$ should equal $C_0 = 1$, we take the \textbf{negative} branch of the $\pm$ (this ensures $C(x)$ is finite at $x=0$):
...
\[
\sqrt{\,1 - 4x\,} \;=\; 1 \;-\; 2x \;-\; 2x^2 \;-\; 4x^3 \;-\; 10x^4 \;-\; \cdots\,,
\] 
