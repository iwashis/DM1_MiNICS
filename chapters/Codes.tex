...
\subsection*{Definition (Linear codes)}
...
The function $\zeta$ is called a linear encoder, if it is a linear mapping between the vector spaces $\mathbb{Z}_p^k$ and $\mathbb{Z}_p^n$. 
...
The image of $\mathbb{Z}_p^k$ under $\zeta$, denoted as $C = \text{Im}(\zeta)$, is a subspace of $\mathbb{Z}_p^n$.\\\\
Hence:
...
\\\\Proof:\\To prove that $C$ is a linear code, we must demonstrate that it is a subspace of $\mathbb{Z}_p^n$. 
...
Hence, $C$ is a linear code. $\square$
...
\subsection*{Remark}
Let 
\\$\zeta: \mathbb{Z}_p^k \to \mathbb{Z}_p^n$ be a linear encoding function defining a linear code $C$. 
...
\subsection*{Definition (standard form)}
We say that a generating matrix $G$ is in \textbf{standard form} if is of the form:
$$G = [I_k | P]$$
where $I_k$ is the $k \times k$ identity matrix and $P$ is a $k \times (n-k)$ matrix.
\subsection*{Remark}
If G is in standard form, then:
$$\underbrace{(u1, u2, ..., uk)}_{\text{u}} G = (u1, u2, ..., uk,   ...   )$$
\subsection*{Example}
...
Notice that if we define H = (1, 1, ..., 1), then:
