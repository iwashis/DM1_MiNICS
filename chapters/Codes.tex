...
\[ C = \xi(\mathbb{Z}_p^k) \subseteq \mathbb{Z}_p^n, \] 
is the set of all possible encoded messages and is called the \emph{code}. The elements of $C$ are called \emph{codewords}. Working over the field $\mathbb{Z}_p$ is crucial here, because it ensures we can use the tools of linear algebra and arithmetic modulo $p$ when designing and analyzing codes.
...
\]
where \(\hat{\mathbf{u}} = \eta(\mathbf{r})\) is the decoder’s estimate of the original message. We desire \(\hat{\mathbf{u}} = \mathbf{u}\) even if \(\mathbf{e}\) is nonzero (up to a certain weight). The design of \(C\) and \(\eta\) determines how many errors can be reliably detected or corrected.
...
i.e. the single-symbol message $a$ is repeated $n$ times to form the codeword. The code $C_1 = \{(a,a,\ldots,a) : a \in \mathbb{Z}_p\}$ consists of all $n$-tuples with identical entries. This encoding is one-to-one (different $a$ give different constant tuples).

\medskip

For example, over $\mathbb{Z}_2$ (the binary case), if $a=1$ and $n=5$, the codeword would be $(1,1,1,1,1)$. If a transmission error flips one of these bits (say we receive $(1,1,1,0,1)$), the decoder can notice that this is not a valid codeword in $C_1$ (since not all bits are the same), hence an error is detected. In fact, the repetition code is able to detect up to $n-1$ errors (any change in at most $n-1$ positions will yield a tuple that is not constant and thus not in the code). This code even has error-correcting capability: intuitively, the decoder could guess that the majority value is the intended bit (for example, from $(1,1,1,0,1)$ a reasonable decoded value for $a$ would be 1, since most bits are 1). However, our focus here is on error detection criteria, which we formalize later.
...
\[ C_2 = \{(a_1,\ldots,a_{k-1},a_k) \in \mathbb{Z}_p^k : a_1 + \cdots + a_{k-1} + a_k = 0\}. \] 
...
$C_3=\xi_{3}(\mathbb{Z}_{10}^{10})\subset\mathbb{Z}_{10}^{11}$
is a legitimate code—the \textsc{PESEL} code—whose distance properties
are governed by the checksum just illustrated.
(Note that $\mathbb{Z}_{10}$ is \emph{not} a field, since $10$ is composite;
this example only uses arithmetic modulo $10$, not the linear-algebra tools
that the field structure of $\mathbb{Z}_p$ provides for prime $p$.)
...
\begin{example}[repetition code of length 7]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{theorem}
A code $C$ is able to detect $t$ errors or fewer \emph{iff}
the minimum distance satisfies $d_C\ge t+1$, where
\[
...
\begin{itemize}
  \item \textbf{Detection:} $d_C\ge t+1$ \;($2\ge t+1\Rightarrow t\le1$).
        So the code \emph{detects exactly one error}.
  \item \textbf{Correction:} $d_C\ge 2t+1$ \;($2\ge 2t+1$ impossible for
...
errors, then
\[
  M\bigl(1 + (p-1)\tbinom{n}{1} + (p-1)^2\tbinom{n}{2}
        + \cdots + (p-1)^t\tbinom{n}{t}\bigr)\;\le\;p^{\,n}.
\]
\end{theorem}

\noindent The factor in parentheses is the number of vectors in a Hamming ball of
radius $t$: there are exactly $\binom{n}{i}(p-1)^i$ vectors at distance
$i$ from the centre, since we choose $i$ positions and, in each, one of
the $p-1$ values different from the centre's entry.
...
\[
  M\bigl(1 + (p-1)\tbinom{n}{1} + \cdots + (p-1)^t\tbinom{n}{t}\bigr)=p^{\,n}.
\]
\end{definition}
...
Let $\zeta: \mathbb{Z}_p^k \to \mathbb{Z}_p^n$ be a coding function that maps a message vector of length $k$ to a codeword of length $n$ (where $k < n$).
The function $\zeta$ is called a \emph{linear encoder} if it is a linear mapping between the vector spaces $\mathbb{Z}_p^k$ and $\mathbb{Z}_p^n$.
That is, for all vectors $\mathbf{u}, \mathbf{v} \in \mathbb{Z}_p^k$ and all scalars $\alpha, \beta \in \mathbb{Z}_p$, the function satisfies:
$$\zeta(\alpha \mathbf{u} + \beta \mathbf{v}) = \alpha \zeta(\mathbf{u}) + \beta \zeta(\mathbf{v})$$
The image of $\mathbb{Z}_p^k$ under $\zeta$, denoted as $C = \text{Im}(\zeta)$, is a subspace of $\mathbb{Z}_p^n$, called a \emph{linear code}.
\end{definition}

\begin{remark}
A non-empty subset $C \subseteq \mathbb{Z}_p^n$ is a linear code if and only if it is closed under linear combinations. That is, for all codewords $\mathbf{c}_1, \mathbf{c}_2 \in C$ and all scalars $\alpha, \beta \in \mathbb{Z}_p$:$$\alpha \mathbf{c}_1 + \beta \mathbf{c}_2 \in C$$
\end{remark}

\begin{example}
Consider the repetition code
$$C = \left\{ (\underbrace{a, a, \dots, a}_{n \text{ times}}) \;\middle|\; a \in \mathbb{Z}_p \right\} \subseteq \mathbb{Z}_p^n$$
It is easy to show that $C$ is a linear code.
\end{example}

\begin{proof}
To prove that $C$ is a linear code, we must demonstrate that it is a subspace of $\mathbb{Z}_p^n$.
We will use the combined subspace criterion. 
Let $\mathbf{c}_1, \mathbf{c}_2$ be any two codewords from $C$, and let $\alpha, \beta$ be any two scalars from the finite field $\mathbb{Z}_p$. 
By definition of the set $C$, since $\mathbf{c}_1 \in C$ and $\mathbf{c}_2 \in C$, there must exist two scalars $a_1, a_2 \in \mathbb{Z}_p$ such that:
$$\mathbf{c}_1 = (a_1, a_1, \dots, a_1)$$
$$\mathbf{c}_2 = (a_2, a_2, \dots, a_2)$$
...
Let us define a new scalar $a_3 = \alpha a_1 + \beta a_2$. Since $\mathbb{Z}_p$ is a field, it is closed under addition and multiplication, meaning that $a_3 \in \mathbb{Z}_p$.
Substituting $a_3$ back into our vector, we get:
$$\alpha \mathbf{c}_1 + \beta \mathbf{c}_2 = (\underbrace{a_3, a_3, \dots, a_3}_{n \text{ times}})$$
Since this resulting vector consists of the same scalar $a_3$ repeated $n$ times, where $a_3 \in \mathbb{Z}_p$, it perfectly matches the definition of the set $C$. 
Therefore:
$$\alpha \mathbf{c}_1 + \beta \mathbf{c}_2 \in C$$
Because the linear combination of any two codewords remains within $C$, the set $C$ is a valid subspace of $\mathbb{Z}_p^n$.
Hence, $C$ is a linear code.
\end{proof}

\begin{example}
The general parity-check code, formally defined as the set
$$C = \left\{ \left( a_1, a_2, \dots, a_{n-1}, -\sum_{i=1}^{n-1} a_i \right) \;\middle|\; a_i \in \mathbb{Z}_p \right\} \subseteq \mathbb{Z}_p^n,$$
is also a linear code.
\end{example}

\begin{proof}
Let $\mathbf{c} = (c_1, c_2, \dots, c_n)$ be an arbitrary vector in $\mathbb{Z}_p^n$.
By definition, $\mathbf{c}$ belongs to $C$ if and only if its components satisfy the relation:
...
$$C = \left\{ \mathbf{c} \in \mathbb{Z}_p^n \;\middle|\; \sum_{i=1}^{n} c_i = 0 \right\}$$

From fundamental linear algebra, the set of all solutions to any system of homogeneous linear equations over a field always forms a vector subspace of the surrounding vector space.
\end{proof}

\begin{remark}
Let $\zeta: \mathbb{Z}_p^k \to \mathbb{Z}_p^n$ be a linear encoding function defining a linear code $C$.
Then, there exists a $k \times n$ matrix $G$ with entries in $\mathbb{Z}_p$ such that for every message $\mathbf{u} \in \mathbb{Z}_p^k$, the encoding function can be presented as:
$$\zeta(\mathbf{u}) = \mathbf{u}G$$
The matrix $G$ is called the \textbf{generator matrix} of the linear code $C$.
\end{remark}

\begin{remark}
Let $C$ be a linear code and let $G$ be its generating matrix.
Then, every codeword $\mathbf{c} \in C$ can be expressed as a linear combination of the rows of $G$.
$$\mathbf{c} \in C \iff \mathbf{c} = \mathbf{u}G \quad \text{for } \mathbf{u} \in \mathbb{Z}_p^k$$
Rephrased: $$C = \left\{ \mathbf{u}G \;\middle|\; \mathbf{u} \in \mathbb{Z}_p^k \right\}$$
\end{remark}

\begin{example}
$$C = \left\{ (0, 0, 0, 0), (0, 1, 0, 1), (1, 0, 1, 1), (1, 1, 1, 0) \right\} \subseteq \mathbb{Z}_2^4$$
C is a linear code, since it is closed under linear combinations.
Encoding function:
$$\zeta: \mathbb{Z}_2^2 \to \mathbb{Z}_2^4$$
Generating matrix:
$$ G = 
\begin{bmatrix}
0 & 1 & 0 & 1 \\
...
We say that a generating matrix $G$ is in \textbf{standard form} if it is of the form:
$$G = [I_k | P]$$
where $I_k$ is the $k \times k$ identity matrix and $P$ is a $k \times (n-k)$ matrix.
\end{definition}

\begin{remark}
If $G = [I_k | P]$ is in standard form, then the codeword starts with the message itself, followed by $n-k$ redundancy (parity) symbols:
$$\underbrace{(u_1, u_2, \dots, u_k)}_{\mathbf{u}} G = (\underbrace{u_1, u_2, \dots, u_k}_{\mathbf{u}I_k = \mathbf{u}}, \underbrace{\;\ast, \dots, \ast\;}_{\mathbf{u}P}).$$
\end{remark}

\begin{example}
$$C = \left\{ \left( a_1, a_2, \dots, a_{n-1}, -\sum_{i=1}^{n-1} a_i \right) \;\middle|\; a_i \in \mathbb{Z}_p \right\}$$
...
-1 \\ -1 \\ -1 \\ \vdots \\ -1
\end{matrix}
\vphantom{\underbrace{\begin{matrix} 1 \\ 0 \\ 0 \\ \vdots \\ 0 \end{matrix}}_{I_{n-1}}}
\end{array}
\right]$$
...
Let $C$ be a linear code in $\mathbb{Z}_p^n$ with generating matrix $G$ of size $k \times n$.
A matrix $H$ of size $(n-k) \times n$ over $\mathbb{Z}_p$
is called a \textbf{parity-check matrix} for $C$, whenever:
$$\mathbf{c} \in C \iff \mathbf{c}H^T = 0$$
\end{definition}
\subsection*{How to obtain H from G?}
...
$$G\mathbf{x} = \mathbf{0}$$
Since the solution set $V = \{\mathbf{x} \in \mathbb{Z}_p^n : G\mathbf{x}^T = \mathbf{0}^T\}$ forms a subspace of $\mathbb{Z}_p^n$ of dimension $n-k$, it possesses a basis. 
Let $\{\mathbf{h}_1, \mathbf{h}_2, \dots, \mathbf{h}_{n-k}\}$ be a basis of this solution space.
The parity-check matrix $H$ is then formed by taking these basis vectors as its rows:
$$H = \begin{bmatrix} \mathbf{h}_1 \\ \mathbf{h}_2 \\ \vdots \\ \mathbf{h}_{n-k} \end{bmatrix}$$
By construction, $G H^T = \mathbf{0}$, which ensures that $H$ is a valid parity-check matrix for the code $C$.
\subsection*{Decoding linear codes}
Assume C is a linear code with parity-check matrix H. Moreover assume $\mathbf{c}$ is the codeword sent, but $\mathbf{r} = \mathbf{c} + \mathbf{e}$ was the received word, where $\mathbf{e}$ is the error vector. 
Then:
$$\mathbf{r}H^T = (\mathbf{c} + \mathbf{e})H^T = \mathbf{c}H^T + \mathbf{e}H^T = \mathbf{0} + \mathbf{e}H^T = \mathbf{e}H^T$$
The product $\mathbf{s} = \mathbf{r}H^T$ is called the \emph{syndrome} of the received vector $\mathbf{r}$.

\begin{theorem}
If $C$ corrects $t$ errors or fewer and $\mathbf{r} = \mathbf{c} + \mathbf{e}$, where $\mathbf{c} \in C$ is the codeword sent and $\mathbf{e}$ is the error vector, whose weight is at most $t$, then $\mathbf{e}$ is the unique vector of smallest weight among the solutions $\mathbf{x}$ of
$$\mathbf{r}H^T = \mathbf{x}H^T.$$
Hence $\mathbf{e}$ (and with it the sent codeword $\mathbf{c} = \mathbf{r} - \mathbf{e}$) can be determined by finding all solutions of this equation and choosing the one with the smallest weight.
\end{theorem}
...
$$rH^T = (1,1,1,1) \begin{bmatrix} 1 & 0 \\ 1 & 1 \\ 1 & 0 \\ 0 & 1 \end{bmatrix} = [0, 1] \text{ in $\mathbb{Z}_2^2$}$$
...
$$\mathbf{e}_1 = (0, 1, 0, 0)$$
$$\mathbf{e}_2 = (0, 0, 0, 1)$$
Hence, the decoder can choose either $\mathbf{e}_1$ or $\mathbf{e}_2$ as the error vector, and decode $\mathbf{r}$ to $\mathbf{c} = \mathbf{r} - \mathbf{e}_1 = (1,0,1,1)$ or $\mathbf{c} = \mathbf{r} - \mathbf{e}_2 = (1,1,1,0)$, both of which are valid codewords in $C$. (The ambiguity does not contradict the theorem: this code has minimum distance $2$, so it corrects $t=0$ errors, while here the error has weight $1$.)
\end{example}
\end{document}