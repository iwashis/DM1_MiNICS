...
A \textbf{walk} in a graph \(G\) is a sequence of vertices \(v_0, v_1, \dots, v_k\) such that \(v_i v_{i+1} \in E(G)\) for every \(0 \le i < k\); the number \(k\) of edges traversed is the \emph{length} of the walk. A \textbf{path} is a walk whose vertices are pairwise distinct. We say that vertices \(u\) and \(v\) are \emph{connected by a path} if there exists a path \(v_0, v_1, \dots, v_k\) with \(v_0 = u\) and \(v_k = v\).
...
Let \(G\) be a graph with vertices \(V(G) = \{v_1, v_2, \dots, v_n\}\). Then the sequence \( (\text{deg}_G(v_1),\dots, \text{deg}_G(v_n)) \), sorted in non-increasing order, is called the \textbf{degree sequence} of \(G\).
...
\begin{itemize}
    ...
    \item \(E_1 \subseteq E(G) \cap \binom{V_1}{2}\)
\end{itemize}
(The second condition says that \(E_1\) consists of edges of \(G\) whose both endpoints lie in \(V_1\); without it, \((V_1,E_1)\) need not be a graph at all.)
