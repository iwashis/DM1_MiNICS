...
Sometimes we only permute $k$ of the $n$ elements, where $1 \le k \le n$ (this notion already appeared in the chapter on counting). The number of ways to do this is the \emph{falling factorial}, denoted $n^{\underline{k}}$, and can be found by thinking:
\[
...
\]
(We avoid the also-common notation $P(n,k)$ for this quantity, since in the chapter on partitions $P(n,k)$ denotes the set of number partitions of $n$ into $k$ parts.)
...
\bigl|A_i\bigr| \;=\; (n-1)!,
\]
since if we fix one position $i$, then we permute the remaining $n-1$ elements freely. In general, for distinct indices $i_1 < i_2 < \cdots < i_k$,
\[
...
Since the partial sums \(\sum_{k=0}^{n} \frac{(-1)^k}{k!}\) converge to \(e^{-1}\) as \(n \to \infty\), we conclude that
\[
...
\section{A Recurrence Relation}
We can also show that $D(n)$ satisfies the recurrence
\[
D(n) \;=\; (n-1)\bigl(D(n-1) + D(n-2)\bigr),
\quad \text{with } D(1)=0, \,D(2)=1.
\]
One way to see this: consider where $1$ goes in a derangement of $\{1,2,\ldots,n\}$. It can go to any of $n-1$ positions. If $1$ goes to position $j$, then either (i)~the element $j$ goes to position $1$ (a swap), which reduces the problem to deranging the remaining $n-2$ elements, or (ii)~the element $j$ does \emph{not} go to position~1. In case~(ii) we must place the $n-1$ elements $\{2,\ldots,n\}$ into the $n-1$ positions $\{1,\ldots,n\}\setminus\{j\}$, where each element $i \neq j$ is forbidden from its own position~$i$, and the element $j$ is forbidden from position~$1$. Relabeling position~$1$ as ``position $j$'' makes every element forbidden from exactly the position carrying its own label, so these placements are precisely the derangements of $n-1$ elements, of which there are $D(n-1)$. Combining the two cases for each of the $n-1$ choices of $j$ yields the recurrence.
\end{document}